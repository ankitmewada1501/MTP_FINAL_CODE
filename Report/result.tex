...
\begin{figure}[H]
\centering
\includegraphics[width=0.95\linewidth]{PARETO_COMP.png}
...
\begin{itemize}
    \item In the Cost vs.\ Coverage plane, both algorithms produce
          ...
          $\sim$49\%) by including very small low-cost layouts,
          while MOPSO concentrates its archive in the
          medium-to-high coverage band (74--100\%).
    \item In the Cost vs.\ Net ROI plane, both algorithms produce
          comparable maximum ROI ($\sim$4.0--4.4) at low cost,
...
\begin{itemize}
    \item NSGA-II produces 80 non-dominated solutions with a
          coverage range of 49.2\%--100\% and a cost range of
          $\text{\rupee}86$\,L--$\text{\rupee}396$\,L. The
          per-solution Net ROI ranges from 2.06 to 4.06, reflecting
          genuine trade-off diversity across deployment scales.
    \item MOPSO produces 100 archive solutions with a coverage
          range of 74.0\%--100\% and a cost range of
          $\text{\rupee}39$\,L--$\text{\rupee}400$\,L. The
          per-solution Net ROI ranges from 2.01 to 4.44; the
    ...
    \item In the Net ROI vs.\ Mean Wait Time plane, all
          ...
          $\sim$6.3\,min, and no Pareto-optimal layout violates
...
\midrule
Hypervolume (HV, normalised)        & 0.9592  & 1.0310  & Higher $\uparrow$ \\
Generational Distance (GD)          & $3.36 \times 10^{4}$ & $5.20 \times 10^{3}$ & Lower $\downarrow$ \\
Spread ($\Delta$)                   & 0.653   & 0.872   & Lower $\downarrow$ \\
Median $W_q$ (min)                  & 1.20    & 1.95    & Lower $\downarrow$ \\
Max Coverage (\%)                   & 100.0   & 100.0   & Higher $\uparrow$ \\
Max Net ROI                         & 4.06    & 4.44    & Higher $\uparrow$ \\
\bottomrule
\end{tabular}
\end{table}
...
$\text{\rupee}2.5$\,Cr--$\text{\rupee}4.0$\,Cr, with mean queue
...
\midrule
Coverage ($f_2$)              & 100.0      & \%        \\
Total CAPEX ($f_1$)            & 2.45--4.00 & Crore INR \\
Net ROI ($f_3$)                & 2.51--3.28 & --        \\
Mean Queue $W_q$ ($f_4$)        & 0.27--1.86 & minutes   \\
Mean Pareto wait (full front)   & 0.18--6.32 & minutes   \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{figure}[H]
\centering
\includegraphics[width=1\linewidth]{queue-time.png}
\caption{Queue waiting time distribution (mean $W_q$ per Pareto
         ...
         line: median (NSGA-II: 1.20\,min; MOPSO: 1.95\,min).}
...
\begin{itemize}
    \item NSGA-II solutions have a median real wait time of
          1.20\,min, ranging from 0.23 to 3.75\,min, all
          comfortably below the 20-minute user-tolerance threshold.
    \item MOPSO solutions have a slightly higher median real wait
          of 1.95\,min, with the full range spanning
          0.18--6.32\,min. The slightly higher median is
...
\begin{figure}[H]
\centering
\includegraphics[width=1\linewidth]{NSGA2-conv.png}
\caption{NSGA-II convergence --- Hypervolume (HV, normalised) per
         generation. Final HV $= 0.9592$. The fastest growth occurs
...
\begin{figure}[H]
\centering
\includegraphics[width=1\linewidth]{MOPSO-conv.png}
\caption{MOPSO convergence --- Hypervolume (HV, normalised) per
         iteration. Final HV $= 1.0310$. The leader-following
...
\begin{figure}[H]
\centering
\includegraphics[width=1\linewidth]{mopso-rate.png}
...
\begin{itemize}
    ...
    \item Despite MOPSO's marginally higher final HV, NSGA-II
          ...
          $0.653$ vs.\ $0.872$), so the two algorithms remain
          complementary.
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{Profit distribution:} The annual-profit
          ...
          $\text{\rupee}1.3$\,Cr--$\text{\rupee}11.7$\,Cr per
          year, with MOPSO reaching the upper end of the tail.
\end{itemize}
...
\begin{itemize}
    ...
    \item The Net ROI objective (annual profit divided by capex)
          ...
          the union archive range from 2.01 to 4.44, with the
