...
def plot_pareto_comparison(nsga2_obj, mopso_obj, out_path):
    ...
    with plt.rc_context(PLT_STYLE):
        ...
        fig.suptitle("Pareto Front Comparison: NSGA-II vs MOPSO", fontsize=23,
        ...
        for ax, n2x, n2y, mox, moy, xl, yl in pairs:
            ...
            ax.set_xlabel(xl, fontsize=17); ax.set_ylabel(yl, fontsize=17)
            ax.legend(fontsize=16); ax.grid(True, alpha=0.3)
        plt.tight_layout()
...
def plot_queue_distribution(nsga2_obj, mopso_obj, out_path,
    ...
    with plt.rc_context(PLT_STYLE):
        ...
        for ax, wq_arr, color, title in [
            ...
            ax.set_title(title, color=color, fontsize=20, fontweight="bold")
            ax.set_xlabel("Mean Waiting Time (minutes)", fontsize=18)
            ax.set_ylabel("Density", fontsize=18)
            ax.legend(fontsize=17); ax.grid(True, alpha=0.3)
        plt.tight_layout()
        fig.savefig(out_path, bbox_inches="tight", dpi=200)
        plt.close(fig)
...
def plot_profit_uncertainty(nsga2_obj, mopso_obj, out_path):
    ...
    with plt.rc_context(PLT_STYLE):
        ...
        ax.set_title("Annual Profit Distribution Across Pareto Solutions",
                     color="#111111", fontsize=20, fontweight="bold")
        ax.set_xlabel("Annual Profit (₹ Crores)", fontsize=18)
        ax.set_ylabel("Density", fontsize=18)
...
def plot_scalability(scalability_data, out_path):
    ...
    with plt.rc_context(PLT_STYLE):
        ...
        ax.set_xlabel("Number of Candidate Locations", fontsize=18)
...
def plot_nsga2_individual(nsga2_obj: np.ndarray, nsga2_rt: float, out_path: str):
    ...
    with plt.rc_context(PLT_STYLE):
        ...
        fig.suptitle("NSGA-II Individual Pareto Front Results",
                     color="#111111", fontsize=23, fontweight="bold", y=0.98)
        obj = nsga2_obj.copy()
        cost =  obj[:, 0] / 1e5          # → Lakhs
        ...
        for ax, x, y, xl, yl, col in pairs:
            ...
            ax.set_xlabel(xl, fontsize=18, color="#333333")
            ax.set_ylabel(yl, fontsize=18, color="#333333")
            ax.tick_params(colors="#111111", labelsize=17)
        ...
        fig.text(0.5, 0.01,
                 ...
                 ha="center", color="#333333", fontsize=17)
...
def plot_nsga2_convergence(history: list, out_path: str):
    ...
    with plt.rc_context(PLT_STYLE):
        ...
        ax.set_title("NSGA-II Convergence — Hypervolume (HV) per Generation",
                     color="#111111", fontsize=22, fontweight="bold", pad=12)
        ax.set_xlabel("Generation", fontsize=19, color="#333333")
        ax.set_ylabel("Hypervolume (f₁ vs f₂ space)", fontsize=19, color="#333333")
        ax.legend(fontsize=17, loc="lower right")
        ax.grid(True, alpha=0.7)
        # Annotate key values
        ax.annotate(f"HV₀ = {history[0]:.3e}",
                    xy=(1, hv_mono[0]), xytext=(len(gens)//8, hv_mono[0]*0.7),
                    color="#555555", fontsize=17,
...
def plot_mopso_convergence(history: list, out_path: str):
    ...
    with plt.rc_context(PLT_STYLE):
        ...
        ax.set_title("MOPSO Convergence — Hypervolume (HV) per Iteration",
                     color="#111111", fontsize=22, fontweight="bold", pad=12)
        ax.set_xlabel("Iteration", fontsize=19, color="#333333")
        ax.set_ylabel("Hypervolume (f₁ vs f₂ space)", fontsize=19, color="#333333")
        ax.legend(fontsize=17, loc="lower right")
        ax.grid(True, alpha=0.7)
        ax.annotate(f"HV₀ = {history[0]:.3e}",
...
def plot_mopso_convergence_rate(history: list, out_path: str):
    ...
    with plt.rc_context(PLT_STYLE):
        ...
        fig.suptitle("MOPSO Convergence Rate Analysis (Fix 6)",
                     color="#111111", fontsize=22, fontweight="bold")
        hv_mono = np.maximum.accumulate(history)
        ...
        ax1.set_xlabel("Iteration", color="#333333", fontsize=18)
        ax1.set_ylabel("Hypervolume", color="#333333", fontsize=18)
        ax1.set_title("Cumulative HV Growth", color="#111111", fontsize=20)
        ...
        ax2.set_xticklabels(labels, rotation=45, ha="right", fontsize=16, color="#111111")
        ax2.set_ylabel("HV Gain per 50 Iters", color="#333333", fontsize=18)
        ax2.set_title("HV Improvement Rate — Convergence Evidence", color="#111111", fontsize=20)
        ...
        ax2.set_xlabel(f"50-iter Window | Verdict: {verdict}", color="#333333", fontsize=18)
